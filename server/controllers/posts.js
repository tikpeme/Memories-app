...
export const updatePost = async(req,res) =>{
    ...
    if(!mongoose.Types.ObjectId.isValid(_id)) return res.status(404).send('No post with that id') //check to see if "_id" is really a mongoose object Id
...
export const deletePost =  async (req, res) => {
    const {id} = req.params
    console.log('Delete')
    if(!mongoose.Types.ObjectId.isValid(id)) return res.status(404).send('No post with that id') //check to see if "_id" is really a mongoose object Id
